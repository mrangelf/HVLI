...
enoe_2025_t3 <- fusion_enoe(
  anio = 2023, #sustituir por 2023 y 2024 para años anteriores
  trimestre = 3,
  rapida = FALSE   
)
...
enoe_analitica <- enoe_2025_t3 %>%
  ...
  select(
    ...
    ent, # En 2023 y 2024 se llama ent, en 2025 se llama ent
    est, 
    sex, mujer, hombre,
...
precariedad_cat_est <- enoe_analitica %>%
  ...
  count(ent, precariedad_categoria, wt = fac_tri, name = "poblacion") %>%
  group_by(ent) %>%
  slice_max(poblacion, n = 1) %>%
  ungroup() %>%
  select(ent, precariedad_categoria)
...
tabla_jornaleras_estatal <- ingreso_med_est %>%
  left_join(horas_mean_est,        by = "ent") %>%
  left_join(sobrejornada_est,      by = "ent") %>%
  left_join(sin_ss_est,            by = "ent") %>%
  left_join(precariedad_est,       by = "ent") %>%
  left_join(precariedad_cat_est,   by = "ent") %>%
  left_join(pob_est,               by = "ent")
...
write_csv(
  tabla_nacional,
  "tabla_nacional_jornaleria_agricola_2023t3.csv"
)

# --- Composición interseccional del sector agropecuario ---
write_csv(
  tabla_agro_composicion,
  "tabla_composicion_agro_sexo_jornaleria_2023t3.csv"
)

# --- Tabla estatal: mujeres jornaleras agrícolas ---
write_csv(
  tabla_jornaleras_estatal,
  "tabla_estatal_mujeres_jornaleras_agricolas_2023t3.csv"
)
